...
public class ParserTest {
    ...
    protected void test() {
        for (final Op<TExpression> test : tests) {
            ...
            final TrippleExpression expression = parse(test.name);
            for (int i = 0; i < 10; i++) {
                for (int j = 0; j < 10; j++) {
                    for (int k = 0; k < 10; k++) {
    ...
    protected TrippleExpression parse(final String expression) {
        try {
            return new ExpressionParser().parse(expression);
        } catch (final Exception e) {
            throw new AssertionError("ExpressionParser failed", e);
        }
    }
    ...
    private void check(final int[] vars, final TrippleExpression expression, final Either<Reason, Integer> answer) {
        try {
